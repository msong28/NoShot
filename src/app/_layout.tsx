    ...
    <Stack screenOptions={{ headerShown: false, contentStyle: { flex: 1 } }}>
      ...
      <Stack.Protected guard={!!session && !!profile}>
        ...
        {/* MOD-05: reachable once signed in, but admin's own layout
            ...
            A real path segment ("admin", not a parenthesized "(admin)"
            group), since a parenthesized group's index.tsx would silently
            collide with (tabs)/index.tsx on the same "/" route -- both are
            simultaneously present for any signed-in user under this same
            guard, and that collision caused an infinite redirect loop. */}
