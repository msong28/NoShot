...
- **One-person settlement for `participant_submission` bets** (migration `20260726090000_one_person_settlement.sql`). For a plain 1v1 bet — the only kind the current create flow produces — whoever reports the outcome settles it outright: `submit_bet_result()` now finalizes immediately (posting the ledger obligation) and pushes a "Bet settled" notification to the other side, instead of the old two-step submit → confirm/dispute flow. **Consequence, accepted by product:** either party can unilaterally declare the result (including declaring themselves the winner), which creates a real ledger debt with no in-app dispute path — a trust-based model deemed appropriate for a friends app. Scope is deliberately narrow: `judge` and `group_vote` bets keep their original multi-step confirm/dispute/vote/fallback flow untouched, so those RPCs still exist and are still reachable for those methods. The web `bet-detail` UI dropped its now-unreachable Confirm/Dispute sections; the root Expo app's settle UI was **not** updated in the same pass (web-only change) and still renders those sections, though they're unreachable for participant-submission bets.
