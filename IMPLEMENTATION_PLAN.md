...
|---|---|---|---|
| 0 | Repo & tooling foundation | — | §9.1, §14.1, §14.3 |
...
- I can do directly: initial git commit of the current scaffold; add `.env.example` (no secrets); install `@supabase/supabase-js`, `@tanstack/react-query`, `expo-secure-store`; add Jest + `jest-expo` + RNTL config with one smoke test; add/verify ESLint + Prettier config (`expo lint` currently has no visible config file — confirm and fix); add a GitHub Actions workflow for lint/typecheck/test; replace `NativeTabs` with the JS custom tab bar (Decision #1) as inert placeholders for the 5 tabs; expand `constants/theme.ts` with the Partiful-inspired palette and a first pass of shared primitives (`Button`, `Card`, `Badge`).
- Needs you at a dashboard: create a GitHub remote if you want CI to actually run (or I can leave the workflow file ready and you push when ready).
- Credentials needed from you: none yet.
- Done when: `npm run lint`, `tsc --noEmit`, and `npm test` all pass in CI on a pushed branch; app still boots on iOS/Android/web with the new tab shell.
...
## Recommended first milestone: Milestone 0

Nothing in Milestone 0 needs Supabase or any external account — it's pure repo hygiene and scaffold work, so it can start immediately. Concretely, Milestone 0 will:

1. Make the initial git commit of the current scaffold (nothing is committed yet).
2. Add `.env.example`, install `@supabase/supabase-js`, `@tanstack/react-query`, `expo-secure-store`.
3. Add Jest (`jest-expo`) + React Native Testing Library with one passing smoke test.
4. Verify/add ESLint + Prettier config (currently no visible eslint config file despite the `lint` script).
5. Add a GitHub Actions workflow (lint, typecheck, test) — will only actually run once you have a remote.
6. Replace the experimental `NativeTabs` layout with a custom JS 5-tab bar (Home, Groups, Add, Activity, Account) as styled placeholders (Decision #1).
7. Expand the design tokens toward the black-base/vibrant-accent Partiful direction and stand up a first `src/components/ui` primitive set (`Button`, `Card`, `Badge`).

I have **not** started this yet — awaiting your go-ahead per your instruction to stop after presenting the first milestone.
